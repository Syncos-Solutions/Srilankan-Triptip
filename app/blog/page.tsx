...
// ============================================================================
// SKELETON — shown while data loads (matches card dimensions exactly)
// ============================================================================
const SkeletonCard: React.FC<{ tall?: boolean }> = ({ tall = false }) => (
  <div className="animate-pulse">
    <div
      className="bg-[#e8e4df] w-full mb-4"
      style={{ aspectRatio: tall ? '4/5' : '16/10' }}
    />
    <div className="h-px w-8 bg-[#e8e4df] mb-4" />
    <div className="h-5 bg-[#e8e4df] w-3/4 mb-2" />
    <div className="h-4 bg-[#e8e4df] w-full mb-1" />
    <div className="h-4 bg-[#e8e4df] w-4/5 mb-5" />
    <div className="flex items-center gap-2">
      <div className="w-7 h-7 bg-[#e8e4df]" />
      <div className="h-3 w-20 bg-[#e8e4df]" />
    </div>
  </div>
);

const FeaturedSkeleton: React.FC = () => (
  <div className="animate-pulse w-full" style={{ height: 'clamp(460px, 55vw, 680px)', background: '#e8e4df' }} />
);

// ============================================================================
// FEATURED POST CARD COMPONENT — unchanged design
// ============================================================================
const FeaturedCard: React.FC<{ post: BlogPostCard }> = ({ post }) => {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: 'linear-gradient(135deg, rgba(94,23,235,0.22), rgba(24,0,173,0.18))', opacity: hovered ? 1 : 0 }}
      />
      ...
      <div className="absolute top-8 left-8 right-8 flex items-center justify-between">
        ...
        <span className="text-[10px] tracking-widest text-white/50 uppercase">Featured Story</span>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-8 sm:p-10 lg:p-14">
        <div
          ...
          style={{ background: 'linear-gradient(to right, #5e17eb, #1800ad)', width: hovered ? '80px' : '40px' }}
        />
        <h2
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <div
            ...
            style={{ color: hovered ? '#fff' : 'rgba(255,255,255,0.6)', gap: hovered ? '12px' : '8px' }}
          >
            Read Story
...
// ============================================================================
// GRID POST CARD COMPONENT — unchanged design
// ============================================================================
const GridCard: React.FC<{ post: BlogPostCard; delay?: number; inView: boolean }> = ({
  post, delay = 0, inView,
}) => {
  const [hovered, setHovered] = useState(false);
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden" style={{ aspectRatio: '16/10' }}>
        ...
        />
        <span
          className="absolute top-4 left-4 text-[9px] font-bold tracking-[0.3em] uppercase px-3 py-1.5 text-white"
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-400">{formatBlogDate(post.published_at)}</p>
            </div>
          </div>
          <div
...
// ============================================================================
// SIDE STORY CARD COMPONENT — unchanged design
// ============================================================================
const SideCard: React.FC<{ post: BlogPostCard; index: number; inView: boolean }> = ({
  post, index, inView,
}) => {
  const [hovered, setHovered] = useState(false);
  ...
  return (
    ...
    >
      <span
        ...
        style={{ fontFamily: "'Syne', sans-serif", color: hovered ? '#5e17eb' : '#e8e4df' }}
      >
...
const EmptyState: React.FC = () => (
  <div className="flex flex-col items-center justify-center py-32 text-center">
    <div className="w-1 h-16 mb-8" style={{ background: 'linear-gradient(to bottom, #5e17eb, #1800ad)' }} />
    <h3 className="text-2xl font-black text-gray-900 tracking-tight mb-3" style={{ fontFamily: "'Syne', sans-serif" }}>
      Stories Coming Soon.
    </h3>
    <p className="text-sm text-gray-500 font-light max-w-xs leading-relaxed">
      Our travel architects are writing. The first stories will appear here shortly.
    </p>
  </div>
);
...
const BlogPage: React.FC = () => {
  ...
  const [allPosts,        setAllPosts]        = useState<BlogPostCard[]>([]);
  ...
  // ── Fetch all published posts once on mount ──────────────────
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
  ...
  const categories = [
    'All',
    ...Array.from(new Set(rest.map(p => p.category))).sort(),
  ];
    ...
    >
      ...
      >
        ...
        <div className="relative z-10 max-w-[1400px] mx-auto">
          <div className={`flex items-center gap-3 mb-10 transition-all duration-700 ${heroRef.inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            ...
            <span className="text-xs tracking-widest text-gray-400 uppercase">Travel Journal</span>
          </div>

          <h1
            className={`font-black tracking-tighter text-gray-900 leading-[0.93] mb-6 transition-all duration-700 delay-100 ${heroRef.inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
            style={{ fontFamily: "'Syne', sans-serif", fontSize: 'clamp(44px,7.5vw,110px)' }}
          >
            Stories That
            <br />
          ...
          <div className={`flex flex-col sm:flex-row sm:items-end justify-between gap-6 transition-all duration-700 delay-200 ${heroRef.inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            ...
            <div className="text-right flex-shrink-0">
              <span className="text-xs tracking-[0.25em] text-gray-400 uppercase">
                {loading ? '— Stories' : `${allPosts.length} Stories Published`}
              </span>
            </div>
          </div>
        </div>
          ...
          <div className="max-w-[1400px] mx-auto flex items-center overflow-x-auto">
            {categories.map(cat => (
              <button
                ...
                style={{ color: activeCategory === cat ? '#5e17eb' : '#999', fontFamily: "'DM Sans', sans-serif" }}
      ...
      <div className="bg-[#ffffff] px-6 sm:px-10 lg:px-12 py-16 lg:py-24">
        <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-16 lg:gap-20">
            ...
            {loading ? (
              ...
              <>
                ...
                {visibleCount < filtered.length && (
                  <div className={`mt-16 flex items-center gap-6 transition-all duration-700 delay-500 ${gridRef.inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <aside ref={sideRef.ref as React.RefObject<HTMLDivElement>} className="space-y-12">
            {/* Must-reads */}
            <div>
              <div className={`flex items-center gap-3 mb-6 transition-all duration-700 ${sideRef.inView ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`}>
                <div className="w-1 h-6 bg-[#5e17eb]" />
                <span className="text-xs font-bold tracking-[0.3em] uppercase text-[#5e17eb]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                  Must Reads
                </span>
              </div>
              {loading
                ? [0, 1, 2, 3].map(i => (
                    <div key={i} className="animate-pulse flex gap-4 py-5 border-b border-[#e8e4df]">
                      <div className="w-10 h-10 bg-[#e8e4df] flex-shrink-0" />
                      <div className="flex-1">
                        <div className="h-3 bg-[#e8e4df] w-1/3 mb-2" />
                        <div className="h-4 bg-[#e8e4df] w-full mb-1" />
                        <div className="h-3 bg-[#e8e4df] w-1/2" />
                      </div>
                    </div>
                  ))
            ...
            {/* Newsletter CTA — static, always shown */}
            <div
              className={`p-8 transition-all duration-700 delay-300 ${sideRef.inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
              style={{ background: 'linear-gradient(135deg, #5e17eb, #1800ad)' }}
            >
              <div className="w-8 h-px bg-white/40 mb-6" />
              <h4 className="text-xl font-black text-white tracking-tight leading-tight mb-3" style={{ fontFamily: "'Syne', sans-serif" }}>
                Get Stories<br />Before Everyone Else.
              </h4>
              ...
              <div className="flex flex-col gap-3">
                ...
                />
                <button type="button" className="w-full py-3 bg-white text-[#5e17eb] text-xs font-black tracking-[0.2em] uppercase transition-opacity hover:opacity-90">
                  Subscribe
                </button>
              </div>
            </div>
            ...
            {!loading && categories.length > 1 && (
              <div className={`transition-all duration-700 delay-400 ${sideRef.inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {categories.slice(1).map(cat => (
                    <button
                      key={cat}
                      type="button"
                      onClick={() => { setActiveCategory(cat); setVisibleCount(10); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
      ...
      <div className="bg-[#e8e4df] px-6 sm:px-10 lg:px-12 py-20 lg:py-28">
        <div className="max-w-[1400px] mx-auto flex flex-col lg:flex-row items-start lg:items-center justify-between gap-10">
          <div>
            <span className="text-xs font-bold tracking-[0.3em] uppercase text-[#5e17eb] block mb-4">Ready to Go?</span>
            <h3
